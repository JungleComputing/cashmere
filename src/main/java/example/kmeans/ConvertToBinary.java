...
public class ConvertToBinary {
    ...
    public static void writeArray(File dir, float[] f, String name)
	    throws Exception {
	...
	if (args.length < 3) {
	    throw new Error(
		    "Usage: java ConvertToBinary <centerFile> <pointsDir> <targetDir>");
	}
	centerFile = args[0];
	pointsDir = args[1];
	...
	if (dest.exists()) {
	    if (!dest.isDirectory()) {
		throw new Error("Destination " + destinationDir
			+ " exists and is not a directory");
	    }
	} else {
	    if (!dest.mkdirs()) {
		throw new Error("Could not create destination dir "
			+ destinationDir);
	    }
	}
